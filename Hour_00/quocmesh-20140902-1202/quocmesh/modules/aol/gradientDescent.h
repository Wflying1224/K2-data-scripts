...
protected:
  ...
  void calcTaudAndDescentDir( VectorType &CurrentPosition, aol::Vector<RealType> &Tau, DerivativeType &DescentDir, DerivativeType &OldGradient, DerivativeType &OldDirection, const int Iterations ) const{
#ifdef VERBOSE
    cerr << "Applying DE ";
#endif
...
class SemiImplicitGradientDescentHelper{
  typedef typename VectorType::Field RealType;
public:
  SemiImplicitGradientDescentHelper() {}
  virtual ~SemiImplicitGradientDescentHelper() {}
  virtual MatrixType* assembleSystemMatrix( const VectorType &Position, const RealType tau ) const = 0;
  virtual void applyImplicitMetric( const VectorType &Position, VectorType &Rhs ) const = 0;
  virtual void smoothDirection( const VectorType &CurrentPosition, const RealType Sigma, VectorType &Direction ) const = 0;
};

template <typename RealType, typename VectorType, typename MatrixType, typename DerivativeType = VectorType>
class SemiImplicitGradientDescentBase
: public GradientDescentBase<RealType, VectorType, DerivativeType > {
  const SemiImplicitGradientDescentHelper<VectorType, MatrixType> &_helper;
public:
  SemiImplicitGradientDescentBase( const aol::Op<VectorType, aol::Scalar<RealType> > &ExplicitEnergyPart,
                                   const aol::Op<VectorType, DerivativeType> &ExplicitEnergyPartDerivative,
                                   const SemiImplicitGradientDescentHelper<VectorType, MatrixType> &Helper,
                                   const int MaxIterations = 50,
                                   const RealType StartTau = aol::ZOTrait<RealType>::one,
                                   const RealType StopEpsilon = aol::ZOTrait<RealType>::zero )
  : GradientDescentBase<RealType, VectorType, DerivativeType >( ExplicitEnergyPart, ExplicitEnergyPartDerivative, MaxIterations, StartTau, StopEpsilon),
    _helper( Helper )
  {
  }
  virtual ~SemiImplicitGradientDescentBase ()
  {
  }
private:
  virtual void smoothDirection( const VectorType &CurrentPosition, const RealType Sigma, VectorType &Direction ) const{
    _helper.smoothDirection( CurrentPosition, Sigma, Direction );
  }
public:
  // x^{k+1} = x^k - tau * A^{-1} E'[x^k]
  virtual void apply( const VectorType &Arg, VectorType &Dest ) const{
    this->_pVecDE = new VectorType(Arg, aol::STRUCT_COPY);                      // = E'[x^k]
    aol::Vector<RealType> tau(1);
    tau[0] = this->_startTau;
    aol::Scalar<RealType> energyScalar;
    Dest = Arg;
    this->_E.apply(Dest, energyScalar);                             // just for the stopping criterion
    RealType energy = energyScalar[0];
    RealType energyNew = energyScalar[0];

    this->_iterations = 0;

    if (this->checkSaveConditions(this->_iterations))
      this->writeTimeStep( Dest, this->_iterations );     // virtual function to call the timestepSaver
    ...
    do{                                                       // iteration loop
      ...
      makeDescentStep( Dest, tau );

      this->_E.apply(Dest, energyScalar);                           // for the stopping criterion
      energyNew = energyScalar[0];
      ...
      stoppingCriterion = (energy - energyNew > this->_stopEpsilon) && ( this->_iterations < this->_maxIterations);

      if ( (this->_updateFilterWidth == true) && (energy - energyNew) < 0.00001 ){
        this->_filterWidth *= 0.5;
        tau.setAll( 1. );
        if( this->_filterWidth > 0.0001)
          stoppingCriterion = true;
      }

      // After applying postProcess the energy has to be recalculated, otherwise the stopping criterion may fail in the next step
      if( postProcess( Dest, this->_iterations ) ){
        this->_E.apply(Dest, energyScalar);                       // for the stopping criterion
        energyNew = energyScalar[0];
      }

      MatrixType* systemMatrix = _helper.assembleSystemMatrix( Dest, tau[0] );
      VectorType rhs( Dest );
      _helper.applyImplicitMetric( Dest, rhs );

      aol::CGInverse<VectorType> solver( *systemMatrix, 1e-16, 10000 );
      solver.setStopping ( aol::STOPPING_ABSOLUTE );

      solver.apply( rhs, Dest );

    } while ( stoppingCriterion ); // end iteration loop

    cerr << endl;
    cerr << "Descent needed " << this->_iterations << " step(s).\n";
    this->_startTau = tau[0];
    delete this->_pVecDE;
  }
  virtual void applyAdd( const VectorType &/*Arg*/, VectorType &/*Dest*/ ) const{
...
template <typename ConfiguratorType, typename VectorType, typename MatrixType>
...
: public SemiImplicitGradientDescentBase<typename ConfiguratorType::RealType, aol::MultiVector<typename ConfiguratorType::RealType>, MatrixType> {
  typedef typename ConfiguratorType::RealType RealType;
  const typename ConfiguratorType::InitType &_grid;
public:
  SemiImplicitGradientDescent( const typename ConfiguratorType::InitType &Initializer,
                               const aol::Op<aol::MultiVector<RealType>, aol::Scalar<RealType> > &ExplicitEnergyPart,
                               const aol::Op<aol::MultiVector<RealType> > &ExplicitEnergyPartDerivative,
                               const SemiImplicitGradientDescentHelper<aol::MultiVector<RealType>, MatrixType> &Helper,
                               const int MaxIterations = 50,
                               const RealType StartTau = aol::ZOTrait<RealType>::one,
                               const RealType StopEpsilon = aol::ZOTrait<RealType>::zero )
  : SemiImplicitGradientDescentBase<RealType, aol::MultiVector<RealType>, MatrixType>( ExplicitEnergyPart, ExplicitEnergyPartDerivative, Helper, MaxIterations, StartTau, StopEpsilon ),
  _grid( Initializer )
  {
  }
  virtual ~SemiImplicitGradientDescent ()
  {
  }
