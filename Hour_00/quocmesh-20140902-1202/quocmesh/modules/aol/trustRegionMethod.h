...
public:
  ...
  RealType partialFactorizationEigenvalueBound( cholmod_factor* L, cholmod_sparse* H, RealType Lambda ) const {
    ...
    if ( H->sorted && H->packed )
      if ( H->stype > 0 ) // upper triangular part is saved; check last entry of kth column
        pos = reinterpret_cast<int*>( H->p )[k+1] - 1;
      else                // lower triangular part is saved; check first entry of kth column
        pos = reinterpret_cast<int*>( H->p )[k];
    else
      throw aol::Exception( "unexpected matrix type in partial factorization for trust region method" );
    if ( k == reinterpret_cast<int*>( H->i )[pos] )
  ...
  void solveSubProblemLanczos( const ConcatDataType &Gradient, const SecondDerivativeType &Hessian, const RealType Delta, ConcatDataType &Step ) const {
    ...
    DerivativeType g_k( Gradient, aol::DEEP_COPY ),
    ...
    VectorType p_k( v_k, aol::DEEP_COPY ),
    ...
    if ( !interior ) {
      ...
      DerivativeType t_k( Gradient, aol::DEEP_COPY );
      VectorType w_k( Step, aol::STRUCT_COPY ),
  ...
  virtual void apply( const VectorType &Arg, VectorType &Dest ) const {
    ...
    DerivativeType Hs( g, aol::STRUCT_COPY );
