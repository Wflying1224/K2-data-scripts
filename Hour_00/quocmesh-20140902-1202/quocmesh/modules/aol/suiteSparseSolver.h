...
public:
  ...
  inline void setMatrix ( const aol::BlockOpBase<RealType,MatrixType> &BlockMat ) {
      ...
      for ( int localRow = 0; localRow < BlockMat.getReference( row, 0 ).getNumRows(); localRow++, totalRow++ ) {
        ...
        for ( int col = 0; col < BlockMat.getNumCols(); col++ ) {
          ...
          for ( typename vector<typename aol::Row<RealType>::RowEntry >::iterator it = matRow.begin(); it != matRow.end(); ++it ) {
            ...
            colInd.pushBack( colOffset + it->col );
    ...
    _n = aol::Max( rowInd.getMaxValue(), colInd.getMaxValue() ) + 1;
    _Ap.resize( _n + 1 );
    _Ai.resize( val.size() );
    _Ax.resize( val.size() );
    umfpack_di_triplet_to_col( _n, _n, val.size(), rowInd.getData(), colInd.getData(), val.getData(), _Ap.getData(), _Ai.getData(), _Ax.getData(), NULL );

    // perform symbolic and numeric factorization
    if ( _umfpackNumeric != NULL )
      umfpack_di_free_numeric( &_umfpackNumeric );
    void *umfpackSymbolic;
    umfpack_di_symbolic( _n, _n, _Ap.getData(), _Ai.getData(), _Ax.getData(), &umfpackSymbolic, _umfpackControl, _umfpackInfo );
    umfpack_di_numeric( _Ap.getData(), _Ai.getData(), _Ax.getData(), umfpackSymbolic, &_umfpackNumeric, _umfpackControl, _umfpackInfo );
  ...
  virtual void apply ( const aol::MultiVector<RealType> &Arg, aol::MultiVector<RealType> &Dest ) const {
    ...
    umfpack_di_solve( UMFPACK_A, _Ap.getData(), _Ai.getData(), _Ax.getData(), sol.getData(), rhs.getData(), _umfpackNumeric, _umfpackControl, _umfpackInfo );
