  ...
  void draw ( const ScalarArray<RealType, qc::QC_2D> &LevelSetFunction,
    ...
    for ( int y = 0; y < numY; y++ ) {
      for ( int x = 0; x < numX; x++ ) {
        RealType levellineImageValue = aol::Clamp( levellineImage.get ( x, y ), 0., 1. );
        ...
        if( levellineImageValue != 0. ){
          ...
          VImage[ColorChannel%3][y*numX+x]
            = aol::Clamp(vImageMaxValue*levellineImageValue + VImage[ColorChannel%3][y*numX+x], 0., vImageMaxValue );
        }
      }
    }
  }
