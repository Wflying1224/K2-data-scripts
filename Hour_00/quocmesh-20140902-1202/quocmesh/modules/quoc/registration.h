...
template <typename ConfiguratorType, bool ClipCoord = false> class SSDEnergy;
template <typename ConfiguratorType, bool ClipCoord = false> class SSDForce;
...
class SSDEnergy : public aol::FENonlinIntegrationVectorInterface<ConfiguratorType, SSDEnergy<ConfiguratorType, ClipCoord>, ConfiguratorType::Dim > {
public:
  typedef typename ConfiguratorType::RealType RealType;
  ...
  : aol::FENonlinIntegrationVectorInterface<ConfiguratorType, SSDEnergy<ConfiguratorType, ClipCoord>, ConfiguratorType::Dim > ( Grid ),
    _r( Grid, ImR ),
    _t( Grid, ImT ) {}
  ...
  RealType evaluateIntegrand( const aol::auto_container<ConfiguratorType::Dim,aol::DiscreteFunctionDefault<ConfiguratorType> > &DiscFuncs,
    ...
    if ( !qc::transformCoord<ConfiguratorType, ClipCoord> ( this->_initializer, DiscFuncs, El, QuadPoint, RefCoord, transformed_el, transformed_local_coord ) ) {
      return 0;
    }
 ...
 */
template <typename ConfiguratorType, bool ClipCoord>
class SSDForce : public aol::FENonlinVectorOpInterface<ConfiguratorType, ConfiguratorType::Dim, ConfiguratorType::Dim, SSDForce<ConfiguratorType, ClipCoord> > {
public:
  typedef typename ConfiguratorType::RealType RealType;
  const aol::DiscreteFunctionDefault<ConfiguratorType> _r;
  const aol::DiscreteFunctionDefault<ConfiguratorType> _t;
  ...
  SSDForce ( const typename ConfiguratorType::InitType &Grid,
             const aol::Vector<RealType> &ImR,
             const aol::Vector<RealType> &ImT )
  : aol::FENonlinVectorOpInterface<ConfiguratorType,ConfiguratorType::Dim,ConfiguratorType::Dim, SSDForce<ConfiguratorType, ClipCoord> > ( Grid ),
    _r( Grid, ImR ),
    _t( Grid, ImT ) {}
...
public:
  ...
  static RealType normalizeImageForNCC ( const aol::MassOp<ConfiguratorType> &MassOp, const aol::Vector<RealType> &Image, aol::Vector<RealType> &NormalizedImage ) {
    typedef typename ConfiguratorType::RealType RealType;
